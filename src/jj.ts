...
export const getTargetFiles = (changeId: string): string[] => {
	const sumamry = execSync(
		`jj show --no-patch -r ${changeId} -T 'diff.summary()'`,
	);
...
export function splitRevisions(changeId: string, revisions: Revision[]) {
	for (const revision of revisions) {
		execSync(
			`jj split ${revision.files.join(" ")} -r ${changeId} --insert-before ${changeId} -m "${revision.commit_message}"`,
			{
				stdio: "ignore",
			},
		);
